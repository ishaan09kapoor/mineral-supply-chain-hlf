...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating Org0 Identities"
    ...
    infoln "Creating Org1-1 Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-org1-1.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
    ...
    infoln "Creating Org1-2 Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-org1-2.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
    ...
    infoln "Creating Org2-1 Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-org2-1.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    while :
    do
      if [ ! -f "organizations/fabric-ca/org0/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} volume rm compose_rmi.msc.com compose_peer0.org0.msc.com compose_peer0.org1-2.msc.com compose_peer0.org2-1.msc.com
